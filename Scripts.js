// ---------- 1. Load questions ----------
let QUESTIONS = []
// Use the shipped Questions.json filename (case-sensitive on some hosts)
...
document.getElementById("categoryBtn").onclick = () => {
  catSelectSec.classList.remove("hidden")
  menu.classList.add("hidden")
}
document.getElementById("backMenu").onclick = () => {
  catSelectSec.classList.add("hidden")
...
document.getElementById("retry").onclick = () =>
  startQuiz(currentCat, currentNum)
document.getElementById("menuBtn").onclick = document.getElementById(
  "menuBtn2"
).onclick = () => {
...
let quizQuestions = [],
  idx = 0,
  score = 0 // score = number of correct answers
...
function showQuestion() {
  ...
  document.getElementById("qNum").textContent = `Question ${idx + 1} of ${
...
function selectAnswer(chosen, correctIndex, btn) {
  ...
  // Use the currently shown question's explanation (avoid searching the global list)
  feedback.textContent = quizQuestions[idx].explanation || ""
  ...
  saveAnswer(currentCat || "Mixed", chosen === correctIndex)
}
...
function endQuiz() {
  ...
  document.getElementById("score").innerHTML = `
    You got <strong>${score}/${quizQuestions.length}</strong> correct (<strong>${pct}%</strong>).
  `
  saveQuizResult(currentCat || "Mixed", score, quizQuestions.length)
}
...
// ---------- 6. Helpers ----------
function hideAll() {
  ;[menu, catSelectSec, quizSec, resultSec, statsSec].forEach((el) =>
